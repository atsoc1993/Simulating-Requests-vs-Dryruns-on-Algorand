...
def call_and_zero_pay():
    ...
    return Seq([
        ...
        Assert(Gtxn[app_call_index].sender() == Addr("ZNOS2OA62XTLQRNOD4GKDHA4V4QTNHQEEY7YDDVJRINGAGAOZZDSSFHVME")), #Change to address you have access to
...
if __name__ == "__main__":
    ...
    os.makedirs(os.path.join(path, "dryrun/artifacts"), exist_ok=True)

    with open(os.path.join(path, "dryrun/artifacts/contract.json"), "w") as f:
        f.write(json.dumps(contract.dictify(), indent=2))

    with open(os.path.join(path, "dryrun/artifacts/approval.teal"), "w") as f:
        f.write(approval)

    with open(os.path.join(path, "dryrun/artifacts/clear.teal"), "w") as f:
        f.write(clear)